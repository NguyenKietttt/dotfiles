# dotfiles

Personal configs, tools, and AI agent setup.

| Folder | Description |
|---|---|
| [`ai/claude`](ai/claude) | Claude Code settings and a Stop hook for native notifications |
| [`ai/instructions`](ai/instructions) | AI agent instructions (global, Unity project) |
| [`ai/pi`](ai/pi) | Pi coding agent extensions |
| [`ai/skills`](ai/skills) | Reusable agent workflows |
| [`g-helper`](g-helper) | G-Helper config for ROG Zephyrus G14 — power limits, fan curves, 80% battery cap |
| [`git`](git) | Git config with a `dog` alias for a compact, decorated log graph |
| [`husky.NET`](husky.NET) | Format files before commit |
