 ...
 * │  To deny a command: add to DENY section                            │
...
export const BUILTIN_RULES: BashRule[] = [
	...
	{ action: "deny", patterns: ["/^rm\\s+.*(-[rf].*-[rf]|-[rf]{2,}|--recursive.*--force|--force.*--recursive).*$/"] },
	{ action: "deny", patterns: ["/^find.*(-delete|-exec|-execdir).*$/"] },

	// ── Default: deny any unmatched bash command ─────────────────────
	{ action: "deny", patterns: ["*"] },
];
