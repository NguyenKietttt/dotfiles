---
name: unity-manual-code-review
description: Review the changes since a fixed point (commit, branch, tag, or merge-base) along two axes — Standards (does the code follow this repo's documented coding standards?) and Spec (does the code match what the originating issue/spec asked for?).
...
Both axes are done **by you, directly, in this conversation** — no sub-agents. Work Standards to completion, then Spec, so neither pass's findings bleed into the other's judgement before it's formed.
...
3. If nothing is found, ask the user where the spec is. If they say there isn't one, skip the **Spec** pass in step 5 and report "no spec available".
...
### 4. Run the Standards pass

Read the diff yourself (`git diff <fixed-point>...HEAD`). Report — per file/hunk where relevant:
...
Write this up fully before moving to step 5 — don't let Spec findings shape it.

### 5. Run the Spec pass

If no spec was found in step 2, skip this pass and note "no spec available" in the final report.

Otherwise, re-read the diff against the spec's contents. Report:
...
Present the two reports under `## Standards` and `## Spec` headings. Do **not** merge or rerank findings — the two axes are deliberately separate (see _Why two axes_).
